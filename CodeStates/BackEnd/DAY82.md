...
1. 사용자 요구사항 정의서에 따라 기능의 우선순위를 결정한다.
2. 협의된 개발 환경에 간단한 API테스트를 하고 통신 상태를 점검한다.
3. 우선순위에 따른 API 개발 작업을 진행한다.
4. 특정 기능을 수행하는 API설정이 끝났다면 테이블을 어떻게 만들지 고민한 후 테이블을 구성하고 Service와 Repository를 만들어 하나의 기능을 마무리한다.
5. API 기능 외 프로젝트 서비스를 위해 필요한 것들을 고민한다. 스케줄러(배치),외부 API통신
6. Spring Security 도입을 검토하고 개발한다.
7. OAuth2 도입을 검토하고 개발한다.
8. 개발 H2 데이터베이스를 MySQL, PostgreSQL등과 같은 데이터베이스로 교체한다.
