...
오늘은 가상 트위터 게시판를 페어 한 분과 같이 소통하며 구현해 보는 시간을 가졌다.

한명은 코드를 구현해보는쪽(드라이버), 한명은 설명하는쪽(네비게이터)을 맡아 진행하는방식이다.

전날밤 페어 프로그래밍이 어떻게 진행될지 기대되고 궁금하였다

오늘 처음 페어프로그래밍을 하면서 느꼇지만,

구현하는쪽, 설명하는쪽만 정해라는 말만 해두었지, 

사실상 주어진 재료와 설명서뿐 그저 1:1 화상 통화인 이 자리에서 진행 능력은 모두 개인재량에 달려있었다.

다행히도 난 틀이 없이 자유로울때 능력과 생산성이 가장 크게 발휘되는 특성이 있었다.

우선 툴을 지정해야만 했다.

난 화면공유를 통해 1번 코드창 + 2번 그림판(아이패드)를 사용하여 쉽게 생각을 전달하려고 했지만, 상대방의 생각을 실시간으로 전달받기에는 다소 부족함이 있었다.

또한 서로 어제 배운 내용조차 아직 잘 이해한지 모른다고 생각하였기에, 서로의 머리속에 추상화된 개념들을 바탕으로 이미지(와이어프레임)을 그려나갔다.

혹여나 잘못 잡혀있는 개념들을 번갈아가며 서로 바로잡아 주는 방식으로 진행하기로 결정한것이다.

우연히도 페어분이 정말 처음 배우시는분이셔서 내가 도와줄 수 있는 부분이 많았다.

코드를 직접 구현하시면서, 모르시는것이 있으면, 어떤 부분을 이해하지 못하고 계신지 계속해서 추론해보며 설명 능력을 늘릴수 있어서 너무 좋았다.

## 나는 어떻게 상대방의 실수를 바라볼까?
...
“’div’와 ‘class’를 구분하지 못하신것아닐까?”

“우선 따라는가셔야하니 보이시는대로 그냥 코드를 쳐보신게 아닐까?”

즉, div와 class에 대한 개념이 아직 잡혀있지 않으신것같았다.
...
내가 상대방의 인지와 추론 과정중에 어떤 과정을 놓치고 있는걸까?
...
이외에도 id, class 왜 쓰느냐, 탭쓰는게 좋냐 스페이드바 쓰는게 좋냐부터, 정말 많은 질문에 답을 해드리다보니 하루가 끝나있었고 너무 보람찬 하루였었다.
